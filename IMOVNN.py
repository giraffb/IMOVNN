...
class IMOVNN(nn.Module):
    ...
    def forward(self, x_set, mask):
        ...
        return y_set, y, mu_set, logvar_set, mu_z, logvar_z, y_10
    ...
    def train_model(self, train_x_set, train_y, mask, alpha, beta, l_rate):
        self.train()
        
        #revise the learning rate of the feature selective layer
        ...
        optimizer = optim.Adam([{"params": self.feature_selective_layer_0.parameters(), "lr": 20*l_rate},
                                {"params": self.feature_selective_layer_1.parameters(), "lr": 20*l_rate},
                              {"params": base_params}],
                             lr=l_rate)
        ...
        for epoch in range(self.ITERATION):
            ...
            y_set, y, mu_set, logvar_set, mu_z, logvar_z, y_10 = self(train_x_set, mask)
            epoch_LOSS_TOTAL, epoch_LOSS_PRE, epoch_LOSS_KL, epoch_LOSS_PRE_set_all, epoch_LOSS_KL_set_all, LOSS_PRE_set, LOSS_KL_set = self.loss_function(mask, train_y,                       y_set, y, mu_set, logvar_set, mu_z, logvar_z, alpha, beta)
            ...
            if (epoch+1)%50 == 0:
                ...
                for m in range(self.M):
                    print(f'mean max of probabilities {m}:',torch.mean(torch.max(F.softmax(self.feature_selective_layer[m].logits.data, dim = -1), dim = -1)[0]))
    ...
    def predict_acc(self, test_x_set, test_y, mask):
        with torch.no_grad():
            ...
            y_set, y, mu_set, logvar_set, mu_z, logvar_z, y_10 = self(test_x_set, mask)
            pre_y = torch.mean(y_10, dim=0)
            acc = torch.mean((torch.argmax(test_y, dim=1) == torch.argmax(pre_y, dim=1)).float())
...
class feature_selective_layer(nn.Module):
    ...
    def forward(self, X):
        ...
        gumbel = -torch.log(-torch.log(uniform)) # avoid numerical instability
